...
export default function Header() {
  ...
  return (
    <header className={`fixed w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-100 py-3' : 'bg-transparent py-5'}`}>
        ...
        <div className="flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-3 lg:gap-4 group" onClick={() => { closeMenu(); window.scrollTo(0, 0); }}>
            ...
            <div className="hidden sm:block">
              <h1 className="text-2xl font-extrabold text-primary leading-none tracking-tight">High Pressure Sump & Tank</h1>
              <h2 className="text-[15px] font-semibold text-dark tracking-wide mt-1">Cleaning Services</h2>
            </div>
          </Link>
      ...
      <div className={`md:hidden fixed inset-0 z-40 bg-navy/30 backdrop-blur-sm transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={closeMenu}></div>
